...
const schema = Joi.object({
    ...
    role: Joi.string().valid('user', 'admin').required()//changed
});
...
export default async (req, res) => {
    ...
    const { error } = schema.validate({ email, password, name,role });//changed
    ...
    try {
        ...
        if (ifExist) {
            ...
            const createUser = await User.create({ email, name,role, password: hashedPassword });//changed
